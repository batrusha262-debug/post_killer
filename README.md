...
- Workspace и collection создаются и сохраняются через UI. Сохранение request
  drafts и загрузка сохранённых запросов через UI пока не подключены.
...
cd ../..
bash packaging/macos/create_dmg.sh 0.2.12 arm64
```
...
Кнопка шестерёнки в правом верхнем углу открывает настройки: системная,
светлая, тёмная и Slay-тема (розово-лиловые акценты в духе Y2K), компактная
плотность и уменьшение анимации. Смена оформления сохраняет открытые вкладки
и черновики запросов. Настройки сохраняются отдельно от workspace в
`post-killer/appearance.json` в каталоге конфигурации пользователя
